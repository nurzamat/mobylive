...
FROM posts AS a LEFT JOIN users as u ON a.idUser = u.ID LEFT JOIN category as cat ON a.idCategory = cat.ID LEFT JOIN subcategory as subcat ON a.idSubCategory = subcat.ID LEFT JOIN subsubcategory as subsubcat ON a.idSubSubCategory = subsubcat.ID WHERE a.status=0 ORDER BY a.city";
...
?>
    ...
    <div id="contentwrapper">
        <div class="main_content">
            <div class="row-fluid">
                <h3 class="heading">Объявления (на модерации)</h3>
                            ...
                            <tr>
                                ...
                                <td type="select">
                                    <select name="adv_status_<?=$adv->id?>" id="adv_status_<?=$adv->id?>" class="textInput">
                                        <option value="0" <?if ($adv->status == 0) echo "selected";?>>На модерации</option>
                                        <option value="1" <?if ($adv->status == 1) echo "selected";?>>Активный</option>
                                        <option value="2" <?if ($adv->status == 2) echo "selected";?>>Срочно продают</option>
                                    </select>
                                </td>
    ...
    <script type="text/javascript">
        ...
        $('#dt_d tbody tr td[type=select]').live('click', function()
        {
            ...
            $.ajax({
                type	: 'POST',
                cache	: false,
                url		: 'ajax/onModeration_ajax.php',
