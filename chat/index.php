...
$dbHost = "176.126.165.135";
$dbUsername = "user13488_ananas";
$dbPassword = "kol0b0k";
$dbName = "user13488_ananas";
...
switch($action) 
{
	...
	case "authenticateUser":
		...
		{					
			...
			//		it is required to fetch the friend, not the chat_users itself
			...
			$sql = "select u.Id, u.username, (NOW()-u.authenticationTime) as authenticateTimeDifference, u.IP, 
							...
							from chat_friends f
							left join chat_users u on
										u.Id = if ( f.providerId = ".$userId.", f.requestId, f.providerId ) 
							where (f.providerId = ".$userId." and f.status=".USER_APPROVED.")  or 
										 f.requestId = ".$userId." ";
										 
			//$sqlmessage = "SELECT * FROM `chat_messages` WHERE `touid` = ".$userId." AND `read` = 0 LIMIT 0, 30 ";
			
			$sqlmessage = "SELECT m.id, m.fromuid, m.touid, m.sentdt, m.read, m.readdt, m.messagetext, u.username from chat_messages m \n"
    . "left join chat_users u on u.Id = m.fromuid WHERE `touid` = ".$userId." AND `read` = 0 LIMIT 0, 30 ";
					...
					while ($row = $db->fetchObject($result))
					{
						...
						if ($resultmessage = $db->query($sqlmessage))			
							...
							while ($rowmessage = $db->fetchObject($resultmessage))
								...
								$sqlendmsg = "UPDATE `chat_messages` SET `read` = 1, `readdt` = '".DATE("Y-m-d H:i")."' WHERE `chat_messages`.`id` = ".$rowmessage->id.";";
								$db->query($sqlendmsg);
								}
							}
					$out .= "</data>";
	...
	case "signUpUser":
		if (isset($_REQUEST['email']))
		{
			 ...
			 if ($result = $db->query($sql))
			 {
			 		...
			 		{
			 				$sql = "insert into chat_users(username, password, email)
	...
	if ($userId = authenticateUser($db, $username, $password)) 
		...
		if (isset($_REQUEST['to']))
		{
			 ...
			 $sqlto = "select Id from  chat_users where username = '".$tousername."' limit 1";
					...
					if ($resultto = $db->query($sqlto))			
					{
						...
						$sql22 = "INSERT INTO `chat_messages` (`fromuid`, `touid`, `sentdt`, `messagetext`) VALUES ('".$userId."', '".$uto."', '".DATE("Y-m-d H:i")."', '".$message."');";
			...
			{				
				 ...
				 if ($result = $db->query($sql))
				 {
				 		if ($row = $db->fetchObject($result))
				 		{
				 			 ...
				 			 if ($row->Id != $userId)
				 			 {
				 			 		 $sql = "insert into chat_friends(providerId, requestId, status)
	...
	case "responseOfFriendReqs":
		...
		if ($userId != NULL)
		{
			...
			if (isset($_REQUEST['approvedFriends']))
			{
				  ...
				  {
				  	$sqlApprove = "update chat_friends set status = ".USER_APPROVED."
				  					where requestId = ".$userId." and 
				  								providerId in (select Id from chat_users where username in (".$friendNamesQueryPart."));
				  				";		
				  }
			...
			if (isset($_REQUEST['discardedFriends']))
			{
				  ...
				  if ($friendNamesQueryPart != NULL)
				  {
				  	$sqlDiscard = "delete from chat_friends
				  						where requestId = ".$userId." and 
				  									providerId in (select Id from chat_users where username in (".$friendNamesQueryPart."));
				  							";
				  }						
...
function authenticateUser($db, $username, $password)
{
	...
	if ($result22 = $db->query($sql22))
	{
		if ($row22 = $db->fetchObject($result22))
		{
				...
				$sql22 = "update chat_users set authenticationTime = NOW(),
